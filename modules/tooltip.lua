...
local function getPhaseText()
    local phase = bis_currentPhaseId;
    if phase <= 1 then
        return "P1 > P2 > P3";
    elseif phase <= 2 then
        return "P2 > P3 > P4";
...
function BIS:OnGameTooltipSetItem(frame)
    ...
    BIS_LibExtraTip:AddDoubleLine(frame,"Class - Races - Spec", getPhaseText() ,r,g,b, r,g,b, true);    
    BIS_LibExtraTip:AddLine(frame," ",r,g,b,true);
    ...
    for key, value in pairs(BIS_TOOLTIP_RANKING[itemId][suffixId][faction][raid][worldboss][pvp][14]) do
        ...
        if phase <= 1 then
           text = (value.P1).." > "..(value.P2).." > "..(value.P3);
        elseif phase <= 2 then
           text = text..(value.P2).." > "..(value.P3).." > "..(value.P4);
        elseif phase <= 3 then
